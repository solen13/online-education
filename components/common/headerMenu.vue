<template>
  <v-card flat tile color="#F6F6F6" height="60" class="d-flex justify-space-between">
    <v-btn class="hidden-sm-and-up mt-4 ml-2" @click="menuBtn" icon>
      <v-icon>mdi-menu</v-icon>
    </v-btn>

    <h1 class="ml-6 title mt-4">{{ $store.state.lessonsName }}</h1>

    <div>
      <v-menu offset-y rounded="xl">
        <template v-slot:activator="{ attrs, on }">
          <v-btn class="rounded-pill grey lighten-2 btn mt-3" v-bind="attrs" v-on="on">
